...
bought = [
	[0.00838564, 11805.9, -99], [0.00433082, 11314.25, -49], [0.00438647, 11170.71, -49], [0.00494595, 9907.1, -49], [0.00526249, 9311.18, -49], [0.00579132, 8460.94, -49], [0.00511541, 9750.01, -50], [0.001006, 10154, 0]
    ,[0.00449406, 8523.29, -38.4], [0.00577656, 8565.01, -49.6], [0.00290492, 9374.96], [.003,9375], [0.00541882, 9147.01], [0.00539200, 9100.01], [0.00539200, 9100.01], [0.00536428, 9217.71]
]
...
for i in range(len(sold)):
	total.soldBTC = total.soldBTC + sold[i][0]  #will divide this by the ratio of USD/BTC to get USD
	c = sold[i][0] - (sold[i][0]*fee)
	u = u + c
	y = y + sold[i][0]
...
#CALCULATE current NUMBER OF BTC I OWN AND THE current USDed I SPEND ON THEM + SOLD BTC:
	...
	soldUSD = round(-total.soldBTC*current.dollarBit,3)
	...
	#current.BTC = (current.BTC + soldBTC)
	z = (-x*current.dollarBit)+(-y*current.dollarBit)

	print(total.soldBTC*fee*current.dollarBit)
	print(u)
	print(current.USD)
	print(soldUSD)
