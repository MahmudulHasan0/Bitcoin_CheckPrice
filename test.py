...
buy = [[0.00449406, 8523.29, -38.4], [0.00577656, 8565.01, -49.6], [0.00290492, 9374.96], [.003,9375], [0.00541882, 9147.01], [0.00539200, 9100.01], [0.00539200, 9100.01], [0.00536428, 9217.71]]
sold   = [[-0.00438647, 8777.94, 38.4], [-.002,8585.00], [-0.00279132, 8585.00], [-0.00577656, 8875.01], [-0.00544134, 9155.00], [-0.00541882, 9100.00], [-0.00539200, 9217.75], [0.00100000, 9205.00], [0.00253276, 9204.62], [0.00183152,9202.94] ]    
#0: BTC sold 	#1: ratio USD/BTC 	#2: USD gained for memory
...
for i in range(len(bought)):
	total.BTC = total.BTC + bought[i][0] #will multiply this by the ratio of USD/BTC to get USD
	x = x + bought[i][0]
...
for i in range(len(sold)):
	total.sellBTC = total.sellBTC + (-sold[i][0])  #will divide this by the ratio of USD/BTC to get USD
...
#CALCULATE current NUMBER OF BTC I OWN AND THE current USDed I SPEND ON THEM + SOLD BTC:
	p = total.buyBTC*current.dollarBit +total.sellBTC*current.dollarBit
	print("ssssssss:")
	print(total.buyBTC*current.dollarBit )
	print(total.sellBTC*current.dollarBit )
	print(total.sellBTC*current.dollarBit - total.buyBTC*current.dollarBit )
	print()

	soldUSD = round(-total.sellBTC*current.dollarBit,3)
	soldUSD = round(soldUSD - (soldUSD*fee),4) #When i sold my bitcoins, how much USD did i get back? Applying the fee because from BTC to USD a fee is applied
	...
	#current.USD = (current.USD + soldUSD )
	...
	print(current.USD)
	print(soldUSD)
	print(current.USD+soldUSD)
	print()
	print(inPut.USD-current.USD+(u))
	print()
	sys.stdout.write("net USD :  "+str(current.USD))

#PRINT THE current I HAVE INPUT TO SYSTEM (BOUGHT), AND current I HAVE SOLD
	if (firstTimeRunning == True):
		sys.stdout.write("CURRENT INVESTMENT   |    G/L DOLLARS, G/L PERCENTAGE   |   BTC/USD   |   PERCENT CHANGES   ||INPUT:   USD:"+ str(round(inPut.USD,3)) + "   BTC:+"+ str(round(inPut.BTC,9))+"   ||SOLD:   USD:+"+str(soldUSD) + "   BTC:"+str(sellBTC)+"\n\n")
